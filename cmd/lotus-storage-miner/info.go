...
func infoCmdAct(cctx *cli.Context) error {
	...
	ppcet:= types.BigDiv(types.BigMul(lockedFunds.PosDeposits, types.NewInt(1000000)),pow.PosTotalPower)
	fmt.Printf("PosPower: %s / %s (%0.4f%%) \n",
		types.FIL(lockedFunds.PosDeposits).Short(),
		types.FIL(pow.PosTotalPower).Short(),
		float64(ppcet.Int64())/10000)
	fmt.Println()
