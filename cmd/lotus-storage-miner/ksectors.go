...
var ksectorsCmd = &cli.Command{
	Name:  "ksectors",
	Usage: "interact with kak sector store",
	Subcommands: []*cli.Command{
		...
		ksectorsKPledgeCmd,
		ksectorsExtendCmd,
		ksectorsTerminateCmd,
		ksectorsRemoveCmd,
		ksectorsMarkForUpgradeCmd,
		ksectorsStartSealCmd,
		ksectorsSealDelayCmd,
		ksectorsCapacityCollateralCmd,
	},
}

var ksectorsPledgeCmd = &cli.Command{
	Name:  "pledge",
...
var ksectorsKPledgeCmd = &cli.Command{
	Name:  "kpledge",
	Usage: "store random data in a sector",
	Action: func(cctx *cli.Context) error {
		ctx := lcli.ReqContext(cctx)
		...
		ver, err := api.StateNetworkVersion(ctx, head.Key())
		if err != nil {
			return err
		}

		rp, err := miner.PreferredSealProofTypeFromWindowPoStType(ver, mi.WindowPoStProofType)
		if err != nil {
			return err
		}
		si := miner.SectorPreCommitInfo{
			SealProof: rp,
			// miner policy MinSectorExpiration or MaxSectorExpirationExtension
			Expiration: head.Height() + builtin.EpochsInDay*540,
		}
		pc, err := api.StateMinerInitialPledgeCollateral(ctx, maddr, si, head.Key())
		if err != nil {
			return err
		}

		pcd, err := api.StateMinerPreCommitDepositForPower(ctx, maddr, si, head.Key())
		if err != nil {
			return err
		}
		collateral := big.Max(pc, pcd)
		fmt.Printf("Estimated collateral: %s\n", types.FIL(collateral))
		//collateral = big.Mul(collateral,big.NewInt(248)) // for kak mode mul 2.48
		collateral = big.Div(collateral, big.NewInt(100)) // big node support float,so div 100  to get 2.48

		var f float64
		f = 0.000000093132257 //1T成本为100kak，换算1KB为0.000000093kak
		secotrSize, err := rp.SectorSize()
		if err !=nil{
			return err
		}
		secotrSize = secotrSize/1024
		f=float64(secotrSize)*f
		f = f * float64(build.FilecoinPrecision)
		fmt.Printf("Real collateral: %s\n", types.FIL(big.NewInt(int64(f))))
		...
		sectorSize := abi.PoStProofInfos[mi.WindowPoStProofType].SectorSize

		params := miner5.AddKPledgeParams{
			Deposit: collateral,
			Size:    sectorSize,
		}
		...
		msg := &types.Message{
			...
			Method: builtin.MethodsMiner.KPledge,
			Params: enc,
		}
...
var ksectorsStatusCmd = &cli.Command{
	Name:      "status",
...
var ksectorsListCmd = &cli.Command{
	Name:  "list",
	Usage: "List sectors",
	Flags: []cli.Flag{
		&cli.BoolFlag{
...
var ksectorsRefsCmd = &cli.Command{
	Name:  "refs",
...
var ksectorsExtendCmd = &cli.Command{
	Name:      "extend",
...
var ksectorsTerminateCmd = &cli.Command{
	Name:      "terminate",
...
var ksectorsTerminateFlushCmd = &cli.Command{
	Name:  "flush",
...
var ksectorsTerminatePendingCmd = &cli.Command{
	Name:  "pending",
...
var ksectorsRemoveCmd = &cli.Command{
	Name:      "remove",
...
var ksectorsMarkForUpgradeCmd = &cli.Command{
...
var ksectorsStartSealCmd = &cli.Command{
	Name:      "seal",
...
var ksectorsSealDelayCmd = &cli.Command{
	Name:      "set-seal-delay",
...
var ksectorsCapacityCollateralCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		fmt.Printf("Estimated pc: %s\n", types.FIL(pc))
		fmt.Printf("Estimated pcd: %s\n", types.FIL(pcd))
...
var ksectorsUpdateCmd = &cli.Command{
	Name:      "update-state",
