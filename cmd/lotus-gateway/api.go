...
func (a *GatewayAPI) StateMinerLockedFunds(ctx context.Context, m address.Address, tsk types.TipSetKey) (*miner.LockedFunds, error) {
	if err := a.checkTipsetKey(ctx, tsk); err != nil {
		return nil, err
	}
	return a.api.StateMinerLockedFunds(ctx, m, tsk)
}

func (a *GatewayAPI) StateThisEpochReward(ctx context.Context, tsk types.TipSetKey) (*abi.TokenAmount, error) {
	if err := a.checkTipsetKey(ctx, tsk); err != nil {
		return nil, err
	}
	return a.api.StateThisEpochReward(ctx, tsk)
}
