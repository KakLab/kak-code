...
use filecoin_proofs_v1::constants::{
    SectorShape2KiB, SectorShape8GiB, SectorShape32GiB, SectorShape512MiB, SectorShape64GiB, SectorShape8MiB,
};
...
impl Labels {
    fn from_raw<Tree: 'static + MerkleTreeTrait>(
        ...
        match proof {
            ...
            StackedDrg8GiBV1 | StackedDrg8GiBV1_1 => {
                if let Some(labels) = Any::downcast_ref::<RawLabels<SectorShape8GiB>>(labels) {
                    Ok(Labels::StackedDrg8GiBV1(labels.clone()))
                } else {
                    bail!("invalid labels provided")
                }
            }
...
impl<Tree: 'static + MerkleTreeTrait> TryInto<RawLabels<Tree>> for Labels {
    ...
    fn try_into(self) -> Result<RawLabels<Tree>> {
        ...
        match self {
            ...
            StackedDrg8GiBV1(raw) => {
                if let Some(raw) = Any::downcast_ref::<RawLabels<Tree>>(&raw) {
                    Ok(raw.clone())
                } else {
                    bail!("cannot convert 8gib into different structure")
                }
            }
...
impl VanillaSealProof {
    ...
    fn from_raw<Tree: 'static + MerkleTreeTrait>(
        ...
        match proof {
            ...
            StackedDrg8GiBV1 | StackedDrg8GiBV1_1 => {
                if let Some(proofs) =
                    Any::downcast_ref::<Vec<Vec<RawVanillaSealProof<SectorShape8GiB>>>>(proofs)
                {
                    Ok(VanillaSealProof::StackedDrg8GiBV1(proofs.clone()))
                } else {
                    bail!("invalid proofs provided")
                }
            }
...
pub fn fauxrep<R: AsRef<Path>, S: AsRef<Path>>(
    ...
    match sector_size {
        ...
        filecoin_proofs_v1::constants::SECTOR_SIZE_8_GIB => {
            filecoin_proofs_v1::fauxrep::<_, _, filecoin_proofs_v1::constants::SectorShape8GiB>(
                config,
                cache_path,
                replica_path,
            )
...
pub fn fauxrep2<R: AsRef<Path>, S: AsRef<Path>>(
    ...
    match sector_size {
        ...
        filecoin_proofs_v1::constants::SECTOR_SIZE_8_GIB => {
            filecoin_proofs_v1::fauxrep2::<_, _, filecoin_proofs_v1::constants::SectorShape8GiB>(
                cache_path,
                existing_p_aux_path,
            )
        }
...
pub fn unseal_range<T: Into<PathBuf> + AsRef<Path>, R: Read, W: Write>(
    ...
    match sector_size {
        ...
        filecoin_proofs_v1::constants::SECTOR_SIZE_8_GIB => filecoin_proofs_v1::unseal_range::<
            _,
            _,
            _,
            filecoin_proofs_v1::constants::SectorShape8GiB,
        >(
            config,
            cache_path,
            sealed_sector,
...
pub fn generate_piece_commitment<T: Read>(
    ...
    match registered_proof {
        StackedDrg2KiBV1 | StackedDrg8MiBV1 | StackedDrg512MiBV1 | StackedDrg8GiBV1 | StackedDrg32GiBV1
        | StackedDrg64GiBV1 | StackedDrg2KiBV1_1 | StackedDrg8MiBV1_1 | StackedDrg512MiBV1_1
        | StackedDrg8GiBV1_1 | StackedDrg32GiBV1_1 | StackedDrg64GiBV1_1 => {
