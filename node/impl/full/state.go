...
type StateModuleAPI interface {
	...
	StateMinerLockedFunds(ctx context.Context, addr address.Address, tsk types.TipSetKey) (*miner.LockedFunds, error)
	StateThisEpochReward(ctx context.Context, tsk types.TipSetKey) (*abi.TokenAmount, error)
...
func (m *StateModule) StateMinerPower(ctx context.Context, addr address.Address, tsk types.TipSetKey) (*api.MinerPower, error) {
	...
	mp, net, hmp, ppow, err := stmgr.GetPower(ctx, m.StateManager, ts, addr)
	if err != nil {
		return nil, err
	}
...
func (m *StateModule) StateMinerLockedFunds(ctx context.Context, addr address.Address, tsk types.TipSetKey) (*miner.LockedFunds, error) {
	ts, err := m.Chain.GetTipSetFromKey(tsk)
	if err != nil {
		return nil, xerrors.Errorf("loading tipset %s: %w", tsk, err)
	}

	lf, err := stmgr.GetLockedFunds(ctx, m.StateManager, ts.ParentState(), addr)
	if err != nil {
		return nil, err
	}
	return lf, nil
...
func (m *StateModule) StateThisEpochReward(ctx context.Context, tsk types.TipSetKey) (*abi.TokenAmount, error) {
	ts, err := m.Chain.GetTipSetFromKey(tsk)
	if err != nil {
		return nil, xerrors.Errorf("loading tipset %s: %w", tsk, err)
	}

	lf, err := stmgr.GetThisEpochReward(ctx, m.StateManager, ts)
	if err != nil {
		return nil, err
	}
	return &lf, nil
