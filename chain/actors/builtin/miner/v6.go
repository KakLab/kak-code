...
func (s *state6) PosDeposits() (abi.TokenAmount, error) {
...
func (s *state6) LoadSectors(snos *bitfield.BitField) ([]*SectorOnChainInfo, error) {
	...
	if snos == nil {
		...
		var info2 miner6.SectorOnChainInfo
		if err := sectors.ForEach(&info2, func(_ int64) error {
			info := fromV6SectorOnChainInfo(info2)
