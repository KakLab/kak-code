...
func (s *state5) LockedFunds() (LockedFunds, error) {
	return LockedFunds{
		...
		PosDeposits:              s.State.PosDeposits,
	}, nil
}
...
func (s *state5) PosDeposits() (abi.TokenAmount, error) {
	return s.State.PosDeposits, nil
}
...
func (s *state5) LoadSectors(snos *bitfield.BitField) ([]*SectorOnChainInfo, error) {
	...
	if snos == nil {
		...
		var info2 miner5.SectorOnChainInfo
		if err := sectors.ForEach(&info2, func(_ int64) error {
			info := fromV5SectorOnChainInfo(info2)
