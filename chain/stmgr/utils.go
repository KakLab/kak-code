...
func GetPower(ctx context.Context, sm *StateManager, ts *types.TipSet, maddr address.Address) (power.Claim, power.Claim, bool, abi.TokenAmount, error) {
	return GetPowerRaw(ctx, sm, ts.ParentState(), maddr)
}

func GetPowerRaw(ctx context.Context, sm *StateManager, st cid.Cid, maddr address.Address) (power.Claim, power.Claim, bool, abi.TokenAmount, error) {
	act, err := sm.LoadActorRaw(ctx, power.Address, st)
	if err != nil {
		return power.Claim{}, power.Claim{}, false, big.NewInt(0), xerrors.Errorf("(get sset) failed to load power actor state: %w", err)
	}

	pas, err := power.Load(sm.cs.ActorStore(ctx), act)
	if err != nil {
		return power.Claim{}, power.Claim{}, false, big.NewInt(0), err
	}
	...
	if maddr != address.Undef {
		...
		if err != nil || !found {
			return power.Claim{}, tpow, false, big.NewInt(0), err
		}
	...
	ppow, _ := pas.TotalPosPower()

	// get miner pos value
	act, err = sm.LoadActorRaw(ctx, maddr, st)
	if err != nil {
		return power.Claim{}, power.Claim{}, false, big.NewInt(0), xerrors.Errorf("(get sset) failed to load miner actor state: %w", err)
	}
	mas, err := miner.Load(sm.cs.ActorStore(ctx), act)
	if err != nil {
		return power.Claim{}, power.Claim{}, false, big.NewInt(0), err
	}
	mf, err := mas.LockedFunds()
	if err != nil {
		return power.Claim{}, power.Claim{}, false, big.NewInt(0), err
	}
	mpow.PosPower = mf.PosDeposits
	return mpow, tpow, minpow, ppow, nil
}

func GetLockedFunds(ctx context.Context, sm *StateManager, st cid.Cid, maddr address.Address) (*miner.LockedFunds, error) {
	// get miner locked funds
	act, err := sm.LoadActorRaw(ctx, maddr, st)
	if err != nil {
...
func MinerGetBaseInfo(ctx context.Context, sm *StateManager, bcs beacon.Schedule, tsk types.TipSetKey, round abi.ChainEpoch, maddr address.Address, pv ffiwrapper.Verifier) (*api.MiningBaseInfo, error) {
	...
	mpow, _, _, ppow, err := GetPowerRaw(ctx, sm, lbst, maddr)
	if err != nil {
...
func GetThisEpochReward(ctx context.Context, sm *StateManager, tipset *types.TipSet) (abi.TokenAmount, error) {
	// get state
	ract, err := sm.LoadActor(ctx, reward.Address, tipset)
	if err != nil {
		return abi.TokenAmount{}, err
	}
	rst, err := reward.Load(sm.cs.ActorStore(ctx), ract)
	if err != nil {
		return abi.TokenAmount{}, err
	}
	reward, err := rst.ThisEpochReward()
	if err != nil {
		return abi.TokenAmount{}, err
	}
