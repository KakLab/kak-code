	...
	for i, m := range miners {
		// Commit sectors
		{
			for pi, preseal := range m.Sectors {
				...
				//pos := big.NewInt(200)
				//posparams := &miner0.AddPosParams{
				//	Pos: big.Mul(pos, big.NewInt(int64(build.FilecoinPrecision))),
				//}
				//_, err = doExecValue(ctx, vm, minerInfos[i].maddr, m.Worker, pos, builtin0.MethodsMiner.AddPos, mustEnc(posparams))
				//if err != nil {
				//	return cid.Undef, xerrors.Errorf("failed to init pos for genesis miner: %w", err)
				//}
			}
			pos := big.NewInt(200)
			posparams := &miner0.AddPosParams{
